...
class Graf:
    ...
    def fct_euristica_h2(self, nod_info):
        if((nod_info[1] + nod_info[2] != 0) and nod_info[0] == 'est'):
            return 2 * ( ((nod_info[1] + nod_info[2]) - 2) / (Nod.NR_LOCURI_BARCA - 1) + 1)
        if ((nod_info[1] + nod_info[2] != 0) and nod_info[0] == 'vest'):
            return 2 * ( ((nod_info[1] + nod_info[2]) - 1) / (Nod.NR_LOCURI_BARCA - 1) + 2)
        return 0


    def muta_barca(self, nod_info, nr_mis, nr_can):
        nod_info_copy = nod_info[:] #echivalent cu nod_info_copy = nod_info.copy()
        if nod_info[0] == 'est':
            nod_info_copy[0] = 'vest'
            nod_info_copy[1] -= nr_mis
            nod_info_copy[2] -= nr_can
            nod_info_copy[3] += nr_mis
            nod_info_copy[4] += nr_can
        else:
            nod_info_copy[0] = 'est'
            nod_info_copy[1] += nr_mis
            nod_info_copy[2] += nr_can
    ...
    def calculeaza_succesori(self, nod):
        ...
        if nod.info[0] == 'est':
            nr_maxim_misionari = min(nod.info[1], Nod.NR_LOCURI_BARCA) + 1
            nr_maxim_canibali = min(nod.info[2], Nod.NR_LOCURI_BARCA) + 1
        else:
            nr_maxim_misionari = min(nod.info[3], Nod.NR_LOCURI_BARCA) + 1
            nr_maxim_canibali = min(nod.info[4], Nod.NR_LOCURI_BARCA) + 1
        # alegem misionari si canibali sa ii punem in barca:
        for nr_misionari in range(nr_maxim_misionari):
            for nr_canibali in range(nr_maxim_canibali):
                if (nr_misionari >= nr_canibali or nr_misionari == 0)\
                        and 1 <= nr_misionari + nr_canibali <= Nod.NR_LOCURI_BARCA:
                    mutare = self.muta_barca(nod.info, nr_misionari, nr_canibali)
                    if( mutare[1] >= mutare[2] or mutare[1] == 0)\
                            and (mutare[3] >= mutare[4] or mutare[3] == 0):
                        h_curent = self.calculeaza_h(mutare)
                        l_succesori.append( (Nod(mutare, h_curent), 1))
